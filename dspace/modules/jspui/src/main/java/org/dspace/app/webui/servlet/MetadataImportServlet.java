...
public class MetadataImportServlet extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        else if ("confirm".equals(request.getParameter("type")))
        {
            ...
            try
            {
            	String useWorkflowCheckbox = request.getParameter("useWorkflow");
            	Boolean useWorkflow = Boolean.FALSE;
            	if (useWorkflowCheckbox != null) {
            		useWorkflow = new Boolean(useWorkflowCheckbox);
            	}
                MetadataImport mImport = new MetadataImport(context, csv);
                List<BulkEditChange> changes = mImport.runImport(true, useWorkflow, false, false);
