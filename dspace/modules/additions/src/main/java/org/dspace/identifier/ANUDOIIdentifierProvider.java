...
import org.dspace.identifier.doi.ANUDOIConnector;
 ...
 */
public class ANUDOIIdentifierProvider extends FilteredIdentifierProvider {
	 ...
	 * DSpace items.
	 */
	private ANUDOIConnector connector;
	 ...
	 */
	protected ANUDOIIdentifierProvider() {
	}
...
//	}
	@Autowired(required = true)
	public void setANUConnector(ANUDOIConnector connector) {
		this.connector = connector;
	}
...
//		doiRow.setStatus(TO_BE_REGISTERED);
		// Since it doesn't already exist set it to be reserved so a DOI can be created
		doiRow.setStatus(TO_BE_RESERVED);
		try {
			doiService.update(context, doiRow);
		} catch (SQLException sqle) {
			log.warn("SQLException while changing status of DOI for {} to be registered.", dso.getID());
...
//		String doi = doiService.formatIdentifier(identifier);
		...
		identifier = connector.reserveDOI(context, dso, identifier);
		log.info("Identifier is now: {}", identifier);

		doiRow.setDoi(identifier);
//		doiRow.setStatus(IS_RESERVED);
		doiRow.setStatus(TO_BE_REGISTERED);
		doiService.update(context, doiRow);
//		saveDOIToObject(context, dso, doiRow.getDoi());
		
		if (identifier == null) {
			log.error("Identifier is null! Need to return it instead");
		} else {
//			MetadataValue mv = new Metadatavalue();
//			dso.getMetadata().add(e)
		}
	}
...
//		String doi = doiService.formatIdentifier(identifier);
		...
		if (null == doiRow.getDoi()) {
			log.error("Unable to register a null doi for record {}", dso.getID());
			return;
		}
		else {
			log.debug("Doi is {} for {}", doiRow.getDoi(), dso.getID());
		}
...
//			doiRow = doiService.findByDoi(context, doi.substring(DOI.SCHEME.length()));
		...
		if (null == doiRow) {
			log.error("Cannot update metadata for object {}: unable to find it in our db.", dso.getID());
		...
		connector.updateMetadata(context, dso, doiRow.getDoi());
...
//			doiRow.setDSpaceObject(null);
			if (doiRow.getDoi() == null) {
				try {
					log.info("Should be deleting the DOI");
					doiService.delete(context, doiRow);
					return;
				}
				catch (SQLException e) {
					log.warn("SQLException while deleting empty DOI for object", dso.getID());
				}
			}
			if (doiRow.getStatus() == null) {
			...
			try {
				...
				log.warn("SQLException while changing status of object {} to set the status of DOI {} to be deleted.", dso.getID(), doiRow.getDoi());
				throw new RuntimeException(sqle);
			}
		}
...
//		}
	...
	protected DOI loadDOI(Context context, DSpaceObject dso, boolean skipFilter) throws SQLException {
		return doiService.findDOIByDSpaceObject(context, dso);
	}
	...
	public void checkMintable(Context context, Filter filter, DSpaceObject dso) throws DOIIdentifierNotApplicableException {
		...
		if (this.filter != null && contentServiceFactory
			...
			try {
				...
				if (!result) {
					// There needs to be an error here so that it isn't put into the minting pool
//					log.info("Item " + dso.getHandle() + " was evaluated as 'false by the item filters, not minting");
	...
	public void activate(Context context, DSpaceObject dso) throws SQLException {
		...
		if (DELETED == doi.getStatus()) {
			doi.setStatus(TO_BE_ACTIVATED);
		}
		else if (TO_BE_DELETED == doi.getStatus()) {
			doi.setStatus(UPDATE_REGISTERED);
		}
		doiService.update(context, doi);
	}
	...
	public void activateOnline(Context context, DSpaceObject dso) throws IdentifierException, SQLException {
		DOI doi = doiService.findDOIByDSpaceObject(context, dso);
		if (TO_BE_ACTIVATED == doi.getStatus()) {
			connector.registerDOI(context, dso, doi.getDoi());
