...
public class BrandedPreviewJPEGFilter extends MediaFilter {
    ...
    public InputStream getDestinationStream(Item currentItem, InputStream source, boolean verbose)
        ...
        return jpegFilter
            .getThumb(currentItem, source, verbose, xmax, ymax, blurring, hqscaling, brandHeight, brandFontPoint,
                         brandFont);
    }
    ...
    public boolean preProcessBitstream(Context c, Item item, Bitstream source, boolean verbose)
		...
		if (maxFileProcessingSize < source.getSizeBytes()) {
			System.out.println("SKIPPED: bitstream "+source.getID()+" (item: "+item.getHandle()+") because it is too large");
			return false;
		}
