...
public class PubmedUtils
{
    ...
    public static Record convertPubmedDomToRecord(Element pubArticle)
    {
        ...
        if (abstractElement != null)
        {
            ...
            List<Element> abstracts = XMLUtils.getElementList(abstractElement, "AbstractText");
            for (Element abstractEl : abstracts)
            {
            	String label = abstractEl.getAttribute("Label");
            	String abstractValue = abstractEl.getTextContent();
            	if (StringUtils.isNotEmpty(label)) {
            		abstractValue = label + " " + abstractValue;
            	}
        		values.add(new StringValue(abstractValue));
            }
        	record.addField("abstractText", values);
