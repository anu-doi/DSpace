...
public class LargeBrandedPreviewJPEGFilter extends MediaFilter {

	@Override
	public String getFilteredName(String sourceName) {
        return sourceName + ".preview.large.jpg";
	}

	@Override
	public String getBundleName() {
		return "LARGE_BRANDED_PREVIEW";
	}
	...
	public String getDescription() {
		return "Generated Large Branded Preview";
	}
	...
	public InputStream getDestinationStream(InputStream source) throws Exception {
        ...
        float xmax = (float) ConfigurationManager
                .getIntProperty("webui.preview.large.maxwidth");
        float ymax = (float) ConfigurationManager
                .getIntProperty("webui.preview.large.maxheight");
    ...
    public boolean preProcessBitstream(Context c, Item item, Bitstream source)
            throws Exception
    {
    	// There is no point in creating the large preview if the image is 
    	// smaller than the preview anyway
    	InputStream is = source.retrieve();
    	try {
	    	BufferedImage buf = ImageIO.read(is);
	    	float xsize = (float) buf.getWidth(null);
	    	float ysize = (float) buf.getHeight(null);
	    	
	    	// Get the preview image maximum
	        float xmax = (float) ConfigurationManager
