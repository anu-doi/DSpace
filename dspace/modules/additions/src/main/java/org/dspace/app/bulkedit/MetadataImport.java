...
public class MetadataImport
{
    ...
    private Metadatum getDcValueFromCSV(String language, String schema, String element, String qualifier, String value, AuthorityValue fromAuthority) {
        ...
        if (fromAuthority != null) {
            ...
            List<AuthorityValue> byValue = new ArrayList<AuthorityValue>();
            if ("orcid".equals(fromAuthority.getAuthorityType())) {
            	AuthorityValue authValue = authorityValueFinder.findByOrcidID(c, value);
            	if (authValue != null) {
            		byValue.add(authValue);
            	}
            }
            else {
	            AuthorityValue example = fromAuthority.newInstance(value);
	            byValue = authorityValueFinder.findByValue(c, schema, element, qualifier, example.getValue());
            }
