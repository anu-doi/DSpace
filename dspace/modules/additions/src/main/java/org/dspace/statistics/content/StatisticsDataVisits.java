...
public class StatisticsDataVisits extends StatisticsData {
	...
	public Dataset createDataset(Context context, int facetMinCount)
		...
		if (dateFacet != null) {
			...
			/*
			 * for (int i = 0; i < datasetQueries.size(); i++) { DatasetQuery datasetQuery =
			 * datasetQueries.get(i); if (datasetQuery.getQueries().size() != 1) { // TODO:
			 * do this } else { String query = datasetQuery.getQueries().get(0); // Loop
			 * over the queries & do em // ObjectCount[] topCounts =
			 * SolrLogger.queryFacetField(query, ); } }
			 */
			...
			if (datasetQueries.size() == 2) {
				...
				for (int i = 0; i < topCounts1.length; i++) {
					...
					/*
					 * for (int j = 0; j < topCounts2.length; j++) { ObjectCount count2 =
					 * topCounts2[j]; String query = firsDataset.getFacetField() + ":" +
					 * count1.getValues(); // Check if we also have a type present (if so this
					 * should be put into the query if ("id".equals(firsDataset.getFacetField()) &&
					 * firsDataset.getQueries().get(0).getDsoType() != -1) query += " AND type:" +
					 * firsDataset.getQueries().get(0).getDsoType();
					 * 
					 * query += " AND " + secondDataSet.getFacetField() + ":" + count2.getValues();
					 * // Check if we also have a type present (if so this should be put into the
					 * query if ("id".equals(secondDataSet.getFacetField()) &&
					 * secondDataSet.getQueries().get(0) .getDsoType() != -1) query += " AND type:"
					 * + secondDataSet.getQueries().get(0).getDsoType();
					 * 
					 * long count = SolrLogger.queryFacetQuery(query, filterQuery);
					 * 
					 * // TODO: the show total // No need to add this many times // TODO: dit
					 * vervangen door te displayen value if (i == 0) { dataset.setRowLabel(j,
					 * getResultName(count2.getValues(), secondDataSet, context));
					 * dataset.setRowLabelAttr(j, getAttributes(count2.getValues(), secondDataSet,
					 * context));
					 * 
					 * }
					 * 
	...
	/**
	 * Gets the name of the DSO (example for collection: ((Collection)
	 * dso).getname();
	 *
