...
import org.dspace.app.rest.projection.WorkflowProjection;
...
public class DiscoveryRestController implements InitializingBean {
    ...
    public SearchResultsResource getSearchObjects(@RequestParam(name = "query", required = false) String query,
        ...
        try {
        	...
        	if("workflow".equalsIgnoreCase(configuration)) {
        		searchResultsRest = discoveryRestRepository.getSearchObjects(query, dsoTypes, dsoScope,
                        configuration, searchFilters, page, new WorkflowProjection());
        	}
        	else {
        		searchResultsRest = discoveryRestRepository.getSearchObjects(query, dsoTypes, dsoScope,
