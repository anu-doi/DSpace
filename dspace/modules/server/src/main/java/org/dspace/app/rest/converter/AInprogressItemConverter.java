...
public abstract class AInprogressItemConverter<T extends InProgressSubmission,
    ...
    protected void fillFromModel(T obj, R witem, Projection projection) {
        ...
        // info
        if(!projection.getName().equals("workflow")) {
	        if (collection != null) {
	            SubmissionDefinitionRest def = converter.toRest(submissionConfigService.getSubmissionConfigByCollection(collection), projection);
